...
from actividades INNER JOIN categoria ON actividades.act_ctg_id = categoria.ctg_id INNER JOIN sub_categoria ON actividades.act_sctg_id = sub_categoria.sctg_id AND actividades.act_ctg_id = sub_categoria.categoria_ctg_id 
...
INNER JOIN sub_categoria ON actividades.act_sctg_id = sub_categoria.sctg_id AND actividades.act_ctg_id = sub_categoria.categoria_ctg_id 
...
ORDER BY segmentacion.nombre ASC');
	...
	function contador_actividades($inicio)
	{
		$query = $this->db->query('SELECT act_fecha,count(act_fecha) as cantidad FROM `actividades` WHERE act_fecha="' . $inicio . '" ');
		//$numero_filas=$query->num_rows();
		return $query->result();
	}
	function cont_dias($inicio, $termino)
	{
		$this->db->distinct();
		$this->db->select('fecha');

		$this->db->from('actividades_calendario');
		//$this->db->where('fecha BETWEEN '.$inicio.' AND '.$termino.'',"", FALSE);
		$this->db->where('fecha >=', $inicio);
		$this->db->where('fecha <=', $termino);
		$this->db->order_by("fecha", "asc");
		$query = $this->db->get();

		/*	$query=$this->db->query('SELECT fecha FROM actividades_calendario
INNER JOIN actividades  on actividades_calendario.act_id = actividades.act_id
INNER JOIN categoria on actividades.act_ctg_id = categoria.ctg_id
INNER JOIN sub_categoria on actividades.act_sctg_id = sub_categoria.sctg_id
WHERE fecha BETWEEN '.$inicio.' AND '.$termino.' ORDER BY fecha');*/
		//$query=$this->db->query('SELECT distinct(fecha) FROM actividades_calendario WHERE fecha BETWEEN '.$inicio.' AND '.$termino.' ORDER BY fecha');
...
WHERE act_id =' . $id . '');
	...
	function Act_cal($id)
	{
		$query = $this->db->query('SELECT * from actividades_calendario ac, actividades a where ac.id = "' . $id . '" AND ac.act_id = a.act_id');
		return $query->result();
	}
	function  actividadCal_id($id, $fecha_sel)
	{
		$id = $this->db->query('SELECT id from actividades_calendario where act_id = "' . $id . '" AND fecha = "' . $fecha_sel . '" ');



		$res2 = $id->result_array();
	...
	function fecha_act($id)
	{

		$query = $this->db->query('SELECT fecha FROM `actividades_calendario` WHERE act_id = ' . $id . '');
		return $query->result();
	}
	...
	/*horario actividad min max*/
	function det_actividades_calendario($inicio)
	{

		$query = $this->db->query('SELECT min(hr_inicio) as inicio, max(hr_termino) as termino,actividades_calendario.act_id, act_responsable, act_empresa,(act_nsocios+act_nprsns) as total, categoria.ctg_nombre , sub_categoria.sctg_nombre, act_estado,act_evento,act_req,act_ctg_id,fecha
FROM `actividades_calendario`
INNER JOIN actividades  on actividades_calendario.act_id = actividades.act_id
INNER JOIN categoria on actividades.act_ctg_id = categoria.ctg_id
INNER JOIN sub_categoria on actividades.act_sctg_id = sub_categoria.sctg_id
WHERE fecha = "' . $inicio . '" GROUP BY act_id,fecha
...
ORDER BY hr_inicio ASC*/
	...
	function det_actividades_pdf($inicio, $termino)
	{
...
ORDER BY fecha,hr_inicio ASC');
	...
	{
		$query = $this->db->query('SELECT * FROM s_personas WHERE prsn_rut = "' . $rut . '"');
		if (($query->num_rows()) > 0) {
	...
	function TipoPersona($rut)
	{

		$query3 = $this->db->query('SELECT prsn_tipoPersona FROM s_personas WHERE prsn_rut = "' . $rut . '"');
		$res2 = $query3->result_array();
		$result = $res2[0]['prsn_tipoPersona'];
		return $result;
	}
	function BuscarFuncionario($rut)
	{
		$query = $this->db->query('SELECT * FROM funcionario WHERE rut = "' . $rut . '"');
		if (($query->num_rows()) > 0) {
	...
	{
		$insertar =  $this->db->insert('s_personas', $data);
		return $insertar;
	}
	function insertarActividad($data)
	{
		$insertar =  $this->db->insert_batch('actividades_personas', $data);
		return $insertar;
	}
	function buscar_asist($id)
	{
		$query = $this->db->query('SELECT * FROM s_personas s, actividades_personas a WHERE s.prsn_rut = a.act_prsn_rut AND a.id_act = "' . $id . '"');
		if (($query->num_rows()) > 0) {
