...
/*retorno la info de la galeria*/
public function info_edit_galeria($id)
{
	# retorna la info de la galeria segun la actividad
	$resultado = $this->db->query('SELECT * FROM web_galeria WHERE `actividad`='.$id.'');
	return $resultado -> result();
}

/*eliminar la foto de la galeria*/
public function eliminar_jpg($id)
{
	$this->db->where('id',$id);
	$this->db->delete('web_galeria');
}
/*actualizar*/
public function actualizar($datos_noti,$id)
{
	var_dump($datos_noti);
	var_dump($id);
	$this->db->where('id', $id);
    $this->db->update('web_actividades', $datos_noti);
	
	# tomo los datos y los registro
/*
	$consulta = $this->db->query('UPDATE `web_actividades` SET `titulo`= '.$datos_noti->titulo.',`fecha`='.$datos_noti->fecha.',`descripcion`='.$descripcion->descripcion.' ,`instituto`='.$datos_noti->instituto.' WHERE `id`= '.$id.'');
*/
}
