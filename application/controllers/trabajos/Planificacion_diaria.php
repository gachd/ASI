...
class planificacion_diaria extends CI_Controller {
	...
	function planificacion (){
		   ...
		   $semana = array_values(array_unique($array_semana));
		    ...
		    // DEFINO TEMPORADA
		    if($mes > 3 and $mes < 11  ){/*TEMPORADA BAJA*/$temporada=1; $nom_temporada="TEMPORADA BAJA";}else{ /*temporada alta*/ $temporada=2;$nom_temporada="TEMPORADA ALTA";}
    ...
    text-transform: uppercase;">AUN NO SE PUEDE CARGAR, DEBE ESPERAR HASTA EL '.iconv('ISO-8859-1', 'UTF-8', strftime('%A %d/%b/%y',  strtotime("".$newfecha_pl.""))).' .</div>';}
             ...
             $nom_mes = DateTime::createFromFormat('!m', $mes);
             $nom_mesSET = strftime("%B", $nom_mes->getTimestamp());
...
//CARGO LA PLANIFICACION GUARDADA.      
		    ...
		    <thead>
		    <tr>
              <th><h3>'.$nom_mesSET.'</h3> <spa style="color:#ff007f; font-size:10px;">'.$nom_temporada.'</span></th>';
