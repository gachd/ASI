...
function busca_categoria_subcategoria($inicio,$termino,$categoria,$subcategoria){
	...
	if($subcategoria == 0) {
		...
		$query=$this->db->query('select actividades.*,categoria.ctg_nombre,sub_categoria.sctg_nombre
            ...
            INNER JOIN categoria ON actividades.act_ctg_id = categoria.ctg_id 
            INNER JOIN sub_categoria ON actividades.act_sctg_id = sub_categoria.sctg_id
            WHERE (act_fecha_termino <= "'.$termino.'" AND  act_fecha >="'.$inicio.'")  AND act_sctg_id='.$subcategoria.' ORDER BY actividades.act_inicio ASC');
	}
		return $query -> result();
}	
...
function total_subcategoria_mes($inicio,$termino,$categoria){
	$query=$this->db->query('select month(actividades.act_fecha),sub_categoria.sctg_nombre, COUNT(act_id) as total from actividades INNER JOIN categoria ON actividades.act_ctg_id = categoria.ctg_id INNER JOIN sub_categoria ON actividades.act_sctg_id = sub_categoria.sctg_id WHERE (act_fecha_termino <= "'.$termino.'" AND act_fecha >="'.$inicio.'") AND ctg_id='.$categoria.' GROUP BY month(actividades.act_fecha) ORDER BY `sub_categoria`.`sctg_nombre` ASC');
	return $query -> result();
