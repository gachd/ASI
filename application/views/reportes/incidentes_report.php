   ...
   return $dias;
		...
		if(!empty($incidentes)){
                ...
                $fun= $this -> model_report -> getFunID($i -> ri_usuario);							
                            ...
                            if($i -> ri_estado == 0 ){
								$estado='<span class="label label-success">Abierto</span>';} else {$estado='<span class="label label-default">Cerrado</span>
    ';}		  		
				...
				echo'<td class="clickable-row"  id="'.$i->ri_id.'" >'.$dias_transcurridos.'</td>';
				 ...
				 /*eliminar*/
                  if($usuario == $i -> ri_usuario ){echo'<td class="eliminar icono"  id="'.$i->ri_id.'" ><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></td>
				  	...
				  	}else{echo'no hay resultados para su busqueda';}
