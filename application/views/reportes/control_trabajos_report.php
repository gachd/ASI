...
$pila = array();
             ...
             //TH cabecera subcategorias
            foreach($subcategoria as $row_sub){
				  
				$sctgid = $row_sub -> sctg_id;  
                $sctg_nombre = $row_sub -> sctg_nombre;
				$tipo =  $row_sub -> sctg_tipo;
                 
                 $tb_sbcatg = $this -> model_trabajos->trabajos_mes_categoria($sctgid,$mes,$year);
                  $pl_sbcatg = $this -> model_trabajos->totalPlanCategoria($sctgid,$mes,$year);

                  //CUENTO SI LA CATEGORIA TIENE TRABAJOS

                   if((count($tb_sbcatg)<>0) or (count($pl_sbcatg)<>0) ){
                                  array_push($pila,$sctgid);
    ...
    text-align: center;">'.strtoupper($row_sector -> nombre).'</td></tr>';
			...
			foreach($pila as $p){
			        ...
			        if(count($plan_mes)==0){echo'<td style="border: 1px solid #ddd;">&nbsp;</td>'; $cantMES=0;}
							...
							foreach($plan_mes as $pm){
							    $cantdTB=$pm -> cantidad;
							    echo'<td style="border: 1px solid #ddd;">'.$pm -> cantidad.''.$i.'</td>';
							     $sumaP='<td>10</td>';
							 }
